...
_Última atualização: 2026-09-07_ · Repositório: [github.com/PaulinoQuicassa/DevSYNOVAR](https://github.com/PaulinoQuicassa/DevSYNOVAR) (**público** desde 2026-09-07, para desbloquear GitHub Pages gratuito) · Versão web publicada em [paulinoquicassa.github.io/DevSYNOVAR](https://paulinoquicassa.github.io/DevSYNOVAR/) (GitHub Pages — Firebase Hosting foi removido, ver nota de migração abaixo).
...
- **Migração Firebase → Supabase, definitiva (2026-09-06/07)**: o backend de dados deixou de ser Firebase Auth/Firestore e passou a ser 100% Supabase (Postgres + Auth + Realtime). Toda a lógica crítica da fila (tirar/chamar/concluir/transferir/cancelar senha) passou a viver em funções RPC `SECURITY DEFINER` no Postgres, protegidas por RLS com isolamento por filial. `firebase_core`/`firebase_auth`/`cloud_firestore` foram removidos das dependências, `lib/firebase_options.dart` e `android/app/google-services.json` foram apagados, o plugin Gradle do Google Services foi removido. O hosting também saiu do Firebase (Hosting → GitHub Pages, repositório tornado público para isso); o projecto Firebase Cloud (`filacerta-d74f0`) fica só como arquivo do backup do Firestore (Fase 0), pendente de eliminação manual pelo utilizador (a credencial de serviço disponível não tem permissão para apagar o projecto). Detalhe completo da migração (auditoria, schema, RLS, RPCs, Realtime, testes) em `docs/` no repositório irmão `fila-certa-staff`.
